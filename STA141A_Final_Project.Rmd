...
In the pursuit of improving the predictive accuracy of the model, several analysis of data and models were undertaken. Initially, the data set was processed to select relevant predictive features. The data was then split into training and testing sets, with 80% allocated for training and the remainder for testing. To build the prediction model, an xgboost package was operated, a decision that originated from considerations on function performance binary classification tasks. Then, the model was trained on the training data. Once trained, the model was utilized to make predictions on the testing set. The resulting predictions were then evaluated for accuracy, yielding a value of 0.4747 for the area under the ROC curve (AUC). This metric, as referenced in earlier above, serves as a measure of the model's ability, distinguishing between the two variables-success and failure. The obtained AUC value of 0.4747 suggests that the model's performance is only marginally better than random chance.
